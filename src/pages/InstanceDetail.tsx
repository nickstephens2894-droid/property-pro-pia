...
const InstanceDetail = () => {
              ...
              applyPreset(
                {
                  ...
                  ownershipAllocations:
                    instanceData.ownership_allocations as any,
                  ...
                  constructionInterestRate:
                    instanceData.construction_interest_rate,
    ...
    if (isDataLoaded && !loading && isEditMode) {
      ...
      if (keyFieldsChanged) {
        console.log("📝 Property data changed, marking as unsaved:", {
          purchasePrice: {
            old: lastData.purchasePrice,
            new: currentData.purchasePrice,
          },
          constructionValue: {
            old: lastData.constructionValue,
            new: currentData.constructionValue,
          },
          buildingValue: {
            old: lastData.buildingValue,
            new: currentData.buildingValue,
          },
        });
  ...
  useEffect(() => {
    if (
      ...
      const hasValidData =
        propertyData.purchasePrice > 0 || propertyData.constructionValue > 0;
  ...
  const convertPropertyDataToInstance = () => {
    ...
    return {
      ...
      post_construction_rate_reduction:
        propertyData.postConstructionRateReduction || 0.5,
          ...
          applyPreset(
            {
              ...
              postConstructionRateReduction:
                instance.post_construction_rate_reduction,
              constructionProgressPayments:
                instance.construction_progress_payments as any,
              ...
              capitalizeConstructionCosts:
                instance.capitalize_construction_costs,
              constructionEquityRepaymentType:
                instance.construction_equity_repayment_type as "io" | "pi",
              landHoldingInterest: instance.land_holding_interest,
              constructionHoldingInterest:
                instance.construction_holding_interest,
              ...
              depreciationMethod: instance.depreciation_method as
                | "prime-cost"
                | "diminishing-value",
            ...
            instance.property_method as any,
            instance.funding_method as any
          );
        }
      }
    } else {
  ...
  const loanPaymentDetails = useMemo(() => {
    const mainLoanDetails = {
      ...
      ioPayment:
        ((propertyData.loanAmount || 0) * (propertyData.interestRate || 6)) /
        100 /
        12,
  ...
  const calculateDepreciationForYear = useCallback(
    (year: number) => {
      ...
      const buildingDepreciationAvailable =
        propertyData.constructionYear >= 1987;
      const buildingDepreciationAnnual = buildingDepreciationAvailable
        ? buildingValue * 0.025
        : 0;
      ...
      if (fixturesDepreciationAvailable && plantEquipmentValue > 0) {
        if (propertyData.depreciationMethod === "prime-cost") {
          ...
          fixturesDepreciationAnnual =
            remainingValue > 0 ? remainingValue * 0.15 : 0;
        }
      }
      ...
      return {
        ...
        total: Math.max(
          0,
          buildingDepreciationAnnual + fixturesDepreciationAnnual
        ),
      };
    },
    [
      ...
      propertyData.depreciationMethod,
    ]
  );
  ...
  const calculateTotalTaxDifference = (
    propertyTaxableIncome: number,
    year: number
  ) => {
    console.log("🔍 calculateTotalTaxDifference called:", {
    ...
    propertyData.investors.forEach((investor) => {
      ...
      if (ownershipPercentage > 0) {
        const baseIncome =
          (investor.annualIncome + investor.otherIncome) * cpiMultiplier;
        const allocatedPropertyIncome =
          propertyTaxableIncome * ownershipPercentage;
        const taxWithoutProperty = totalTaxAU(
          baseIncome,
          investor.hasMedicareLevy
        );
        const taxWithProperty = totalTaxAU(
          baseIncome + allocatedPropertyIncome,
          investor.hasMedicareLevy
        );
  ...
  const projections = useMemo(() => {
    ...
    if (
      ...
      const capitalisePortion = propertyData.capitalizeConstructionCosts
        ...
        ? Math.max(
            ...
            Math.min(
              1,
              (100 - (propertyData.holdingCostCashPercentage || 0)) / 100
            )
          )
        : 0;

      // Rates during construction
      const mainMonthlyRateConstruction =
        (propertyData.constructionInterestRate ||
          propertyData.interestRate ||
          6.0) /
        100 /
        12;
      const equityMonthlyRateConstruction =
        (propertyData.equityLoanInterestRate || 7.2) / 100 / 12;

      // Main loan: IO during construction
      const mainInterestAccrued =
        funding.mainLoanAmount * mainMonthlyRateConstruction * months;
      ...
      if (equityBalanceTemp > 0) {
        if (propertyData.constructionEquityRepaymentType === "io") {
          equityInterestAccrued =
            equityBalanceTemp * equityMonthlyRateConstruction * months;
          equityInterestCapitalised = equityInterestAccrued * capitalisePortion;
          equityInterestCash =
            equityInterestAccrued - equityInterestCapitalised;
          ...
          for (let m = 0; m < months; m++) {
            ...
            equityBalanceTemp = Math.max(
              0,
              equityBalanceTemp - principal + capInt
            );
      ...
      const constructionEquityPaymentCash =
        equityInterestCash + equityPrincipalPaid;

      // Tax: use existing holdingCosts breakdown (deductible interest proxy)
      const constructionTaxableIncome = -holdingCosts.total;
      const constructionTaxBenefit = -calculateTotalTaxDifference(
        constructionTaxableIncome,
        0
      );
      const constructionAfterTaxCashFlow =
        -(constructionMainPaymentCash + constructionEquityPaymentCash) +
        constructionTaxBenefit;
      ...
      constructionPeriodProjection = {
        ...
        mainLoanIOStatus: "IO",
        equityLoanIOStatus:
          propertyData.constructionEquityRepaymentType === "pi" ? "P&I" : "IO",
        otherExpenses: 0,
        depreciation: 0,
    ...
    for (let year = 1; year <= 40; year++) {
      ...
      const mainIsIOPeriod =
        propertyData.mainLoanType === "io" &&
        year <= (propertyData.ioTermYears || 0);
      const equityIsIOPeriod =
        propertyData.equityLoanType === "io" &&
        year <= (propertyData.equityLoanIoTermYears || 0) &&
        funding.equityLoanAmount > 0;
      ...
      const councilRates =
        (propertyData.councilRates || 0) * inflationMultiplier;
      ...
      const afterTaxCashFlow =
        rentalIncome - otherExpenses - totalLoanPayments + taxBenefit;
      ...
      const totalReturn =
        ...
        (year > 1
          ? propertyValue -
            (propertyData.purchasePrice || totalProjectCost) *
              Math.pow(1 + capitalGrowth, year - 2)
          : 0);
    ...
    holdingCosts,
    depreciation.total,
    calculateTotalTaxDifference,
    totalProjectCost,
  ]);

  // Calculate assumptions for projections
  const assumptions = useMemo(() => {
    const tripletify = (value: any) => ({
      mode: "auto",
      auto: value,
      manual: null,
    });
  ...
  const annualInterest =
    (projections[0]?.mainInterestYear || 0) +
    (projections[0]?.equityInterestYear || 0);
  ...
  const investorTaxResults = useMemo(() => {
    ...
    return propertyData.investors.map((investor) => {
      ...
      const adjustedTotalIncome =
        (investor.annualIncome + (investor.otherIncome || 0)) * cpiMultiplier;
      ...
      console.log(`🧾 Tax calculation for ${investor.name}:`, {
        ...
        calculation: `${taxWithoutProperty} - ${Math.abs(
          investorTaxBenefit
        )} = ${taxWithProperty}`,
      });
    ...
    projections,
    propertyData.investors,
    propertyData.ownershipAllocations,
  ...
  const totalTaxRefundOrLiability = useMemo(() => {
    ...
    console.log("🧾 Tax Refund/Liability Calculation (CORRECTED):", {
      ...
      meaning:
        result < 0
          ? "Tax Savings/Benefit (reduces tax)"
          : "Tax Liability (increases tax)",
  ...
  useEffect(() => {
    if (projections.length > 0 && investorTaxResults.length > 0) {
      console.log("🔍 COMPREHENSIVE TAX VALIDATION:", {
        ...
        investorTaxResultsSum: investorTaxResults.reduce(
          (sum, result) => sum + result.taxDifference,
          0
        ),
        signConventionCheck: {
  ...
  const investmentSummary = useMemo(() => {
    ...
    const weeklyAfterTaxCashFlowSummary =
      (currentYearData?.afterTaxCashFlow ?? 0) / 52;
    const taxDifferenceSummary = -(currentYearData?.taxBenefit ?? 0);
    const taxSavingsTotal = projections
      .slice(0, yearTo)
      .reduce((sum, p) => sum + Math.max(0, p.taxBenefit), 0);
    const cumulativeTaxImpact = projections
      .slice(0, yearTo)
      .reduce((sum, p) => sum + p.taxBenefit, 0);
    ...
    const cumulativeCashContribution = Math.abs(
      Math.min(
        0,
        projections
          .slice(0, yearTo)
          .reduce((sum, p) => sum + p.afterTaxCashFlow, 0)
      )
    );

    // Calculate ROI = Net Equity / Cumulative Cash Contribution * 100
    const roiAtYearTo =
      cumulativeCashContribution > 0
        ? (equityAtYearTo / cumulativeCashContribution) * 100
        : 0;

    const cpiMultiplier =
      yearFrom >= 1 ? Math.pow(1 + 2.5 / 100, yearFrom - 1) : 1;
    const marginalTaxRateSummary =
      propertyData.investors.length > 0
        ? Math.max(
            ...propertyData.investors.map((c) =>
              marginalRateAU((c.annualIncome + c.otherIncome) * cpiMultiplier)
            )
          )
        : 0.325;
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Failed to delete instance. Please try again.",
        variant: "destructive",
      });
    } finally {
    ...
    <div className="min-h-screen bg-background">
      ...
      >
        ...
        <div className={isMobile ? "mb-6" : "mb-8"}>
          ...
          <div
            className={`${isMobile ? "space-y-4" : "flex items-center gap-4"}`}
          >
            {/* Instance Details - Enhanced */}
            <Card
              className={`${isMobile ? "w-full" : "flex-1 min-w-0"} border-2 ${
                isEditMode
                  ? "border-primary"
                  : "border-dashed border-muted-foreground/20"
              } ${
                isEditMode ? "border-primary/50" : "hover:border-primary/30"
              } transition-colors`}
            >
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full ${
                      isEditMode ? "bg-orange-500" : "bg-primary"
                    }`}
                  ></div>
            ...
            <div
              className={`${
                isMobile ? "grid grid-cols-2 gap-3" : "flex gap-3"
              }`}
            >
              {isEditMode ? (
                <>
                ...
                <div className="space-y-6">
                  ...
                  <PropertyCalculationDetails
                    ...
                    funding={{
                      ...
                      cashRequired:
                        totalProjectCost -
                        funding.mainLoanAmount -
                        funding.equityLoanAmount,
                    ...
                    mainLoanPayments={{
                      ...
                      ioTermYears:
                        loanPaymentDetails.mainLoanDetails.ioTermYears,
                      remainingTerm:
                        (propertyData.loanTerm || 30) -
                        loanPaymentDetails.mainLoanDetails.ioTermYears,
                      totalInterest:
                        ...
                        100,
                      currentPayment:
                        loanPaymentDetails.mainLoanDetails.isIO &&
                        loanPaymentDetails.mainLoanDetails.ioTermYears > 0
                          ? loanPaymentDetails.mainLoanDetails.ioPayment
                          : loanPaymentDetails.mainLoanDetails.piPayment,
                      futurePayment:
                        loanPaymentDetails.mainLoanDetails.isIO &&
                        loanPaymentDetails.mainLoanDetails.ioTermYears > 0
                          ? loanPaymentDetails.mainLoanDetails.piPayment
                    ...
                    equityLoanPayments={
                      loanPaymentDetails.equityLoanDetails
                        ? {
                            ioPayment:
                              loanPaymentDetails.equityLoanDetails.ioPayment,
                            piPayment:
                              loanPaymentDetails.equityLoanDetails.piPayment,
                            ioTermYears:
                              loanPaymentDetails.equityLoanDetails.ioTermYears,
                            remainingTerm:
                              (propertyData.equityLoanTerm || 30) -
                              loanPaymentDetails.equityLoanDetails.ioTermYears,
                            totalInterest:
                              ...
                              100,
                            currentPayment:
                              loanPaymentDetails.equityLoanDetails.isIO &&
                              loanPaymentDetails.equityLoanDetails.ioTermYears >
                                0
                                ? loanPaymentDetails.equityLoanDetails.ioPayment
                                : loanPaymentDetails.equityLoanDetails
      ...
      <ConfirmationDialog
        ...
        onCancel={
          deletingInstance ? undefined : () => setDeleteDialogOpen(false)
        }
      />
    </div>
  );
