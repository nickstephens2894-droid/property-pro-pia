...
const AddInstance = () => {
  ...
  const investorTaxResults = useMemo(() => {
    ...
    const totalDeductibleExpenses =
      ...
      ((propertyData.weeklyRent || 0) *
        ...
        100 +
      depreciation.total +
      ((propertyData.loanAmount || 0) * (propertyData.interestRate || 0.06)) /
        100;
  ...
  const totalTaxRefundOrLiability = useMemo(() => {
    return investorTaxResults.reduce(
      (total, result) => total + result.taxDifference,
      0
    );
  }, [investorTaxResults]);
  ...
  const netOfTaxCostIncome = useMemo(() => {
    ...
    const annualInterest =
      ...
      (propertyData.useEquityFunding && equityLoanAmount
        ? (equityLoanAmount * (propertyData.equityLoanInterestRate || 7.2)) /
          100
        : 0);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const instanceData = {
        ...
        ownership_allocations: JSON.parse(
          JSON.stringify(propertyData.ownershipAllocations)
        ),
        ...
        construction_progress_payments: JSON.parse(
          JSON.stringify(propertyData.constructionProgressPayments)
        ),
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder={
                        selectedModel
                          ? selectedModel.name
                          : "Select a property above to auto-populate..."
                      }
                    />
                    {instanceName && (
                  ...
                  {selectedModel ? (
                    instanceName && (
                      <p className="mt-2 text-xs text-muted-foreground">
                        Your instance will be saved as:{" "}
                    ...
                    <p className="mt-2 text-xs text-orange-600">
                      💡 Tip: Select a property above to automatically populate
                      this field and all property details
                    </p>
                  )}
                </CardContent>
              </Card>
            ...
            onApplyProperty={(propertyData: any) => {
              ...
              const mappedData = {
                ...
                isConstructionProject:
                  propertyData.is_construction_project || false,
                ...
                constructionInterestRate:
                  propertyData.construction_interest_rate || 7.0,
              ...
              <div className={isMobile ? "col-span-1" : "col-span-7"}>
                <div className="sticky top-4">
                  <PropertyInputForm
                    ...
                    instanceId={`temp-${Date.now()}`} // Temporary ID for funding management
                  />
                </div>
              </div>
                ...
                <div className="space-y-6">
                  <PropertyCalculationDetails
                    ...
                    totalDeductibleExpenses={(() => {
                      const annualInterest =
                        ((propertyData.loanAmount || 0) *
                          (propertyData.interestRate || 0.06)) /
                        100;
                      const cashDeductions =
                        annualInterest +
                        (propertyData.councilRates || 0) +
