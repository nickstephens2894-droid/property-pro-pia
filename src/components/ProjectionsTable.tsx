...
export const ProjectionsTable = ({
  ...
  const [showCashFlowDetailsMobile, setShowCashFlowDetailsMobile] =
    useState(false);

  const filteredProjections = projections.filter(
    (p) => p.year >= validatedYearRange[0] && p.year <= validatedYearRange[1]
  );
...
const MobileProjectionsView = ({
  ...
  const repaymentsTotal =
    currentProjection.mainLoanPayment + currentProjection.equityLoanPayment;
  const expensesTotal = repaymentsTotal + currentProjection.otherExpenses;
  const nonCashTotal =
    currentProjection.depreciation + Math.max(0, currentProjection.taxBenefit);
  const rentalPct = incomeTotal
    ? (currentProjection.rentalIncome / incomeTotal) * 100
    : 0;
  const repaymentPct = expensesTotal
    ? (repaymentsTotal / expensesTotal) * 100
    : 0;
  const operatingPct = expensesTotal
    ? (currentProjection.otherExpenses / expensesTotal) * 100
    : 0;
  const depreciationPct = nonCashTotal
    ? (currentProjection.depreciation / nonCashTotal) * 100
    : 0;
  const taxBenefitNonCashPct = nonCashTotal
    ? (Math.max(0, currentProjection.taxBenefit) / nonCashTotal) * 100
    : 0;
  const taxTotal = currentProjection.taxBenefit;
  const weeklyCashflow = currentProjection.afterTaxCashFlow / 52;
  const rawEquityPct =
    currentProjection.propertyValue > 0
      ? (currentProjection.propertyEquity / currentProjection.propertyValue) *
        100
      : 0;
  const equityRatio = Math.max(0, Math.min(100, rawEquityPct)); // for progress bar (0-100)
  const rawLvrPct =
    currentProjection.propertyValue > 0
      ? ((currentProjection.mainLoanBalance +
          currentProjection.equityLoanBalance) /
          currentProjection.propertyValue) *
        100
      : 0;
  ...
  const prevValue =
    currentYearIndex > 0
      ? projections[currentYearIndex - 1].propertyValue
      : currentProjection.propertyValue;
  const yoyChange =
    prevValue > 0
      ? ((currentProjection.propertyValue - prevValue) / prevValue) * 100
      : 0;
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold">
                      {formatCurrency(currentProjection.propertyValue)}
                    </span>
                  </div>
                </div>
              </CardContent>
                ...
                <div className="mt-2 space-y-1 text-xs text-muted-foreground">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono">
                      {formatCurrency(assumptions.initialPropertyValue)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`font-mono ${
                        ...
                        currentProjection.propertyValue -
                          assumptions.initialPropertyValue <
                          0
                          ? "text-destructive"
                          : "text-foreground"
                      }`}
        ...
        <Card>
          <Collapsible
            open={showEquityDetails}
            onOpenChange={setShowEquityDetails}
          >
            <CollapsibleTrigger asChild>
              <CardContent className="pt-4 cursor-pointer">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`font-bold ${
                        currentProjection.propertyEquity < 0
                          ? "text-destructive"
                          : "text-foreground"
                      }`}
                    >
              ...
              <CardContent className="pt-0 space-y-3">
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold">
                      {formatCurrency(currentProjection.mainLoanBalance)}
                    </span>
                  </div>
                  {(assumptions.initialEquityLoanBalance as number) > 0 && (
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold">
                        {formatCurrency(currentProjection.equityLoanBalance)}
                      </span>
                    </div>
                  )}
                </div>
                  ...
                  <div className="bg-blue-50 dark:bg-blue-950/20 rounded-lg p-3 mt-2">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm font-medium">
                        Main Loan (
                        {(
                          (assumptions.mainLoanType as string) || "pi"
                        ).toUpperCase()}
                        )
                      </span>
                      <Badge
                        variant={
                          currentProjection.mainLoanIOStatus === "IO"
                            ? "secondary"
                            : "default"
                        }
                      >
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-muted-foreground">
                        {formatCurrency(
                          assumptions.initialMainLoanBalance as number
                        )}{" "}
                        @{" "}
                        {formatPercentage(
                          assumptions.mainInterestRate as number
                        )}
                      </span>
                      <span className="font-bold">
                        {formatCurrency(currentProjection.mainLoanPayment)}
                      </span>
                    </div>
                    {currentProjection.mainLoanIOStatus === "P\u0026I" && (
                      <div className="flex justify-between items-center mt-1">
                        ...
                        <span className="font-mono text-xs">
                          {formatCurrency(currentProjection.mainInterestYear)}
                        </span>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="bg-green-50 dark:bg-green-950/20 rounded-lg p-3 mt-2">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-medium">
                          Equity Loan (
                          {(
                            (assumptions.equityLoanType as string) || "pi"
                          ).toUpperCase()}
                          )
                        </span>
                        <Badge
                          variant={
                            currentProjection.equityLoanIOStatus === "IO"
                              ? "secondary"
                              : "default"
                          }
                        >
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-xs text-muted-foreground">
                          {formatCurrency(
                            assumptions.initialEquityLoanBalance as number
                          )}{" "}
                          @{" "}
                          {formatPercentage(
                            assumptions.equityInterestRate as number
                          )}
                        </span>
                        <span className="font-bold">
                          {formatCurrency(currentProjection.equityLoanPayment)}
                        </span>
                      </div>
                      {currentProjection.equityLoanIOStatus === "P\u0026I" && (
                        <div className="flex justify-between items-center mt-1">
                          ...
                          <span className="font-mono text-xs">
                            {formatCurrency(
                              currentProjection.equityInterestYear
                            )}
                          </span>
                        </div>
                      )}
              ...
              <CardContent className="pt-4 cursor-pointer">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-destructive">
                    -{formatCurrency(Math.abs(expensesTotal))}
                  </span>
                </div>
              </CardContent>
            </CollapsibleTrigger>
              ...
              <CardContent className="pt-0 space-y-2">
                ...
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono text-sm">
                      {formatCurrency(currentProjection.otherExpenses)}
                    </span>
                  </div>
                  <div className="mt-2 space-y-1 pl-6 text-xs text-muted-foreground">
                    <div className="flex items-center justify-between">
                      <span>
                        Property management (
                        {formatPercentage(
                          (assumptions.propertyManagementRate as number) || 0
                        )}
                        )
                      </span>
                      <span className="font-mono">
                        {formatCurrency(
                          currentProjection.rentalIncome *
                            (((assumptions.propertyManagementRate as number) ||
                              0) /
                              100)
                        )}
                      </span>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-mono">
                        {formatCurrency(
                          (assumptions.councilRates as number) *
                            Math.pow(
                              1 +
                                (assumptions.expenseInflationRate as number) /
                                  100,
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-mono">
                        {formatCurrency(
                          (assumptions.insurance as number) *
                            Math.pow(
                              1 +
                                (assumptions.expenseInflationRate as number) /
                                  100,
        ...
        <Card>
          <Collapsible
            ...
            onOpenChange={setShowIncomeDetails}
          >
            <CollapsibleTrigger asChild>
              <CardContent className="pt-4 cursor-pointer">
                <div className="flex items-center justify-between">
              ...
              <CardContent className="pt-0 space-y-2">
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono text-sm">
                      {formatCurrency(currentProjection.rentalIncome)}
                    </span>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="pt-4 cursor-pointer">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`font-bold ${
                        taxTotal >= 0 ? "text-primary" : "text-destructive"
                      }`}
                    >
                      {taxTotal >= 0 ? "+" : "-"}
                      {formatCurrency(Math.abs(taxTotal))}
                    </span>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="pt-0 space-y-2">
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono text-sm">
                      {formatCurrency(currentProjection.depreciation)}
                    </span>
                  </div>
                </div>
        ...
        <Card>
          <Collapsible
            open={showCashFlowDetailsMobile}
            onOpenChange={setShowCashFlowDetailsMobile}
          >
            <CollapsibleTrigger asChild>
              <CardContent className="pt-4 cursor-pointer">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`font-bold ${
                      weeklyCashflow >= 0 ? "text-primary" : "text-destructive"
                    }`}
                  >
                    {weeklyCashflow >= 0 ? "+" : "-"}
            ...
            <CollapsibleContent>
              <CardContent className="pt-0 space-y-3">
                ...
                <div className="flex justify-between items-center pt-2 border-t">
                  <span className="text-sm font-medium">
                    After-tax cash flow (year)
                  </span>
                  <span
                    className={`font-semibold ${
                      currentProjection.afterTaxCashFlow >= 0
                        ? "text-primary"
                        : "text-destructive"
                    }`}
                  >
  ...
  return (
    <div className="overflow-auto">
      <table className="w-full border-collapse">
        <thead>
          <tr>
            <th className="sticky left-0 top-0 bg-background z-20 min-w-[180px] text-left p-3 border-b">
              End of year
            </th>
            {assumptions.isConstructionProject &&
              assumptions.constructionPeriod > 0 && (
                <th className="sticky top-0 bg-muted/30 z-20 text-center font-medium p-3 border-b">
                  Construction
                </th>
              )}
            {projections.map((projection: YearProjection) => (
              <th
                key={projection.year}
                className="sticky top-0 bg-background z-20 text-center min-w-[100px] font-medium p-3 border-b"
              >
                {projection.year === 0
                  ? "Construction"
        ...
        <tbody>
          ...
          <tr className="border-b">
            ...
            {projections.map((projection: YearProjection) => (
              <td
                key={projection.year}
                className="text-center font-mono text-sm p-3"
              >
                {projection.year === 0
                  ? "—"
            ...
            {projections.map((projection: YearProjection) => (
              ...
              >
                {projection.year === 0
                  ? "—"
                  : formatCurrency(
                      projection.mainLoanBalance + projection.equityLoanBalance
                    )}
              </td>
            ))}
          </tr>
          ...
          {showLoanDetails && (
            <tr className="bg-blue-50 dark:bg-blue-950/20 border-b">
              ...
              {projections.map((projection: YearProjection) => (
                <td
                  key={projection.year}
                  className="text-center font-mono text-xs p-3"
                >
                  {projection.year === 0
                    ? "—"
                    : formatCurrency(projection.mainLoanBalance)}
                </td>
              ))}
            </tr>
          )}
              ...
              <tr className="bg-green-50 dark:bg-green-950/20 border-b">
                ...
                {projections.map((projection: YearProjection) => (
                  ...
                  >
                    {projection.year === 0
                      ? "—"
                      : formatCurrency(projection.equityLoanBalance)}
                  </td>
                ))}
              </tr>
            )}
          ...
          <tr className="bg-muted/30 border-y">
            <td className="sticky left-0 bg-muted/30 z-10 font-semibold p-3">
              Expenses
            </td>
          ...
          >
            <td className="sticky left-0 bg-background z-10 font-medium p-3">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="ml-2">
                  ...
                  {projections[0]?.mainLoanIOStatus === "IO" ||
                  projections[0]?.equityLoanIOStatus === "IO"
                    ? "IO"
                    : "P\u0026I"}
                </Badge>
              </div>
            ...
            {projections.map((projection: YearProjection) => (
              ...
              >
                {formatCurrency(
                  projection.mainLoanPayment + projection.equityLoanPayment
                )}
              </td>
            ))}
          </tr>
            ...
            <tr className="bg-blue-50 dark:bg-blue-950/20 border-b">
              ...
              {projections.map((projection: YearProjection) => (
                <td
                  ...
                  className="text-center font-mono text-xs p-3"
                >
                  {projection.mainLoanIOStatus === "P\u0026I"
                    ? formatCurrency(projection.mainInterestYear)
                    : "—"}
                </td>
              ))}
            </tr>
              ...
              <tr className="bg-green-50 dark:bg-green-950/20 border-b">
                ...
                {projections.map((projection: YearProjection) => (
                  <td
                    ...
                    className="text-center font-mono text-xs p-3"
                  >
                    {projection.equityLoanIOStatus === "P\u0026I"
                      ? formatCurrency(projection.equityInterestYear)
                      : "—"}
                  </td>
                ))}
              </tr>
          ...
          <tr
            className="border-b hover:bg-muted/50 cursor-pointer"
            onClick={() => setShowOperatingDetails(!showOperatingDetails)}
          >
            <td className="sticky left-0 bg-background z-10 font-medium p-3">
              <div className="flex items-center gap-2">
                {showOperatingDetails ? (
            ...
            {projections.map((projection: YearProjection) => (
              <td
                ...
                className="text-center font-mono text-sm p-3"
              >
                {formatCurrency(projection.otherExpenses)}
              </td>
            ))}
          </tr>
          ...
          <tr className="border-b">
            ...
            {projections.map((projection: YearProjection) => (
              <td
                ...
                className="text-center font-mono text-sm p-3"
              >
                {formatCurrency(projection.rentalIncome)}
              </td>
            ))}
          </tr>
              ...
              {projections.map((projection: YearProjection) => (
                ...
                >
                  {formatCurrency(
                    (assumptions.councilRates as number) *
                      Math.pow(
                        1 + (assumptions.expenseInflationRate as number) / 100,
                        projection.year - 1
                      )
                  )}
                </td>
          ...
          <tr
            className="border-b hover:bg-muted/50 cursor-pointer"
            onClick={() => setShowDepreciationDetails(!showDepreciationDetails)}
          >
            <td className="sticky left-0 bg-background z-10 font-medium p-3">
              <div className="flex items-center gap-2">
                {showDepreciationDetails ? (
              ...
              <td className="sticky left-0 bg-purple-50 dark:bg-purple-950/20 z-10 pl-8 text-sm p-3">
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>
                        Capital works depreciation: 2.5% annually (prime cost
                        method)
                        <br />
                        Available for buildings constructed after Sept 1987
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              ...
              <td className="sticky left-0 bg-purple-50 dark:bg-purple-950/20 z-10 pl-8 text-sm p-3">
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>
                        Plant & equipment depreciation: 15% using selected
                        method
                        <br />
                        Available for new properties only (purchased after May
                        2017)
                      </p>
                    </TooltipContent>
                  </Tooltip>
          ...
          <tr
            className="bg-muted/50 border-t-2 border-b hover:bg-muted/60 cursor-pointer"
          ...
          >
            ...
            {projections.map((projection: YearProjection) => (
              <td
                key={projection.year}
                className={`text-center bg-muted/50 font-mono text-sm font-bold p-3 ${
                  projection.afterTaxCashFlow / 52 >= 0
                    ? "text-green-600"
                    : "text-destructive"
                }`}
              >
