...
export function useRepository() {
  ...
  const [investors, setInvestors] = useState<Investor[]>(() =>
    lsGet<Investor[]>(LS_KEYS.investors, [])
  );
  const [properties, setProperties] = useState<Property[]>(() =>
    lsGet<Property[]>(LS_KEYS.properties, [])
  );
  const [scenarios, setScenarios] = useState<Scenario[]>(() =>
    lsGet<Scenario[]>(LS_KEYS.scenarios, [])
  );
  ...
  useEffect(() => {
    console.log("Repository useEffect triggered, userId:", userId);
    if (!userId) {
      console.log("No userId, staying on local cache");
      return; // stay on local cache when logged out
    }
    ...
    const loadAll = async () => {
      ...
      const { data: dbInvestors, error: iErr } = await supabase
        .from("investors")
        .select(
          "id, name, annual_income, other_income, non_taxable_income, has_medicare_levy, created_at, updated_at"
        )
      ...
      if (iErr) {
        console.error("Investors query error:", iErr);
      ...
      const { data: dbProperties, error: pErr } = await supabase
        .from("properties")
        .select(
          "id, name, type, purchase_price, weekly_rent, location, notes, owner_user_id"
        )
        .eq("owner_user_id", userId)
        .order("created_at", { ascending: true });

      if (pErr) {
        console.error("Properties query error:", pErr);
        toast.error(`Failed to load properties: ${pErr.message}`);
        // Don't return early - continue with investors
      }
      ...
      if (sErr) {
        console.error("Scenarios query error:", sErr);
        toast.error(`Failed to load scenarios: ${sErr.message}`);
        // Don't return early - continue with investors and properties
      }
      ...
      console.log("Setting state - investors count:", mergedInvestors.length);
    ...
    loadAll().catch((error) => {
      console.error("Unexpected error in loadAll:", error);
      setLoading(false);
    });
  }, [userId]);
  ...
  useEffect(() => {
    ...
    const importLocalData = async () => {
      ...
      const alreadyImported =
        localStorage.getItem(LS_KEYS.importedFlag) === "true";
      if (
        !alreadyImported &&
        (lsGet<Investor[]>(LS_KEYS.investors, []).length ||
          lsGet<Property[]>(LS_KEYS.properties, []).length ||
          lsGet<Scenario[]>(LS_KEYS.scenarios, []).length)
      ) {
        ...
        if (!data) {
          ...
          const { error: insErr } = await supabase
            .from("investors")
            .insert({ id, name: DEFAULT_NAME, owner_user_id: userId });
          if (insErr) {
        ...
        if (localScenarios.length) {
          ...
          if (!attachInvestorId) {
            const { data: defaultInvestor } = await supabase
              .from("investors")
              .select("id")
              .eq("name", DEFAULT_NAME)
              .single();
          ...
          if (attachInvestorId) {
            ...
            const { error } = await supabase
              ...
              .insert(scenarioRows);
            if (error)
              toast.error(`Failed to import scenarios: ${error.message}`);
          }
        }
  ...
  const updateInvestor = useCallback(
    async (id: string, patch: Partial<Investor>) => {
      if (!userId) {
        setInvestors((p) =>
  ...
  const updateProperty = useCallback(
    async (id: string, patch: Partial<Property>) => {
      if (!userId) {
        setProperties((p) =>
          p.map((prop) => (prop.id === id ? { ...prop, ...patch } : prop))
        );
        return;
      }
  ...
  const totalWeeklyRent = useMemo(
    () => properties.reduce((sum, p) => sum + p.weeklyRent, 0),
    [properties]
  );
  ...
  // Helper functions
  const investor = useCallback(
    (id?: string) => investors.find((i) => i.id === id),
    [investors]
  );
...
const RepositoryContext = createContext<ReturnType<
